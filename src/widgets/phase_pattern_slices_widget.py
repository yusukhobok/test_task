...
class PhasePatternSlicesWidget(QtWidgets.QWidget):
    ...
    def plot_slices(self, fi_deg, theta_deg, slice_fi_deg, slice_theta_deg, results, log_scale):
        fi_min, fi_max, theta_min, theta_max = results['limits_deg']

        if self.line_fi is None:
            self.line_fi = self.plot_widget_fi.plot(np.degrees(results['fi_array']), slice_fi_deg, pen=pg.mkPen(color='k'))
        else:
